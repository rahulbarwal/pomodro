<script lang="ts">
  import TimerValEditor from "./TimerValEditor.svelte";
  ...
  {#each properties as property, idx}
    <section class="my-6">
      <TimerValEditor
        ...
        {...property}
        on:increment={() => settingsStore.incrementTimerVal(property.title)}
        on:decrement={() => settingsStore.decrementTimerVal(property.title)}
      />
    </section>
  {/each}
</main>
